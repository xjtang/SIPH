...
for i in $(seq 1 $njob); do
  ...
  qsub -j y -N sdt_$i -V -b y cd /projectnb/landsat/users/xjtang/documents/';' python -m SIPH.models.fusion.area_over_threshold ${overwrite}${recursive}-p $pattern -b $i $njob $ori $des $img
done
