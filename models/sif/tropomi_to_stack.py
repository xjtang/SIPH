""" Module for converting TROPOMI SIF netCDF to stacked image
...
from ...io import tpm2stack
from ...common import constants as cons
from ...common import log, get_files, manage_batch


def tpm_to_stack(pattern, ori, des, overwrite=False, recursive=False,
                    batch=[1,1]):
    ...
    try:
        tpm_list = get_files(ori, pattern, recursive)
        n = len(tpm_list)
    except:
    ...
    if batch[1] > 1:
        ...
        tpm_list = manage_batch(tpm_list, batch[0], batch[1])
        n = len(tpm_list)
    ...
    for tpm in tpm_list:
        log.info('Processing {}'.format(tpm[1]))
        if tpm2stack(os.path.join(tpm[0], tpm[1]), des, overwrite) == 0:
            count += 1
...
if __name__ == '__main__':
    ...
    # print logs
    log.info('Start converting TROPOMI SIF netCDF to stack...')
    ...
    # run function to convert TROPOMI SIF
    tpm_to_stack(args.pattern, args.ori, args.des, args.overwrite,
